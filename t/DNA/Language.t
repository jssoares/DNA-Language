...
ok ( my $lanalysis =  DNA::Language->new( fasta_file => 't/data/fasta_seqs/very_small.fa', mode => 'test', k => 3, rand_range => 4, tnb => 200, host => '', branch => '', rmode => '', test => 1 ), 'Creating language analysis object' );
$lanalysis->run;
...
ok ( $lanalysis =  DNA::Language->new( fasta_file => 't/data/fasta_seqs/slightly_bigger.fa', mode => 'test', k => 3, rand_range => 4, tnb => 200, host => '', branch => '', rmode => '', test => 1 ), 'Creating language analysis object' );
$lanalysis->run;
